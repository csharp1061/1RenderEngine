...
namespace OEngine
{
	...
	*/
	static void register_window_class()
	{
		...
		wc.lpszClassName = L"MyRenderWindow";					//该窗口类的名字
	...
	*/
	...
	int window_init(int width, int height, const char* title)
	{
		...
		//创建窗口
		window->h_window = CreateWindowA(("MyRenderWindow"), title,
	...
	{
		int actualSize = window->height * window->width;
		int frameSize = framebuffer.size();

		assert(actualSize == frameSize);
		...
		for (int i = 0; i < window->height; i++)
		{
			for (int j = 0; j < window->width; j++)
			{
				...
				Vector3 color = framebuffer[i * window->width + j];
				...
				window->window_fb[index] = b;
				window->window_fb[index + 1] = g;
				window->window_fb[index + 2] = r;
				window->window_fb[index + 3] = 255;
			}
		}
		window_display();
	}
	...
	OEngine::Vector2 get_mouse_pos()
	{
		...
		return OEngine::Vector2((float)point.x, (float)point.y);
	}
